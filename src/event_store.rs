...
mod tests {
  ...
  async fn test() {
    ...
    user_account_repository
      .store(&event, user_account.version(), Some(&user_account))
      .await
      .unwrap();

    let mut user_account = user_account_repository
      .find_by_id(&UserAccountId { value: "1".to_string() })
      .await
      .unwrap();
    ...
    user_account_repository
      .store(&event, user_account.version(), None)
      .await
      .unwrap();
    let user_account = user_account_repository
      .find_by_id(&UserAccountId { value: "1".to_string() })
      .await
      .unwrap();
