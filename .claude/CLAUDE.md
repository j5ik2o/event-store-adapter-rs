...
- **Model/backend**: The shipped `.claude/settings.json` sets the orchestrator model by alias only (`"model": "opus[1m]"` — Opus with the 1M-context variant) and pins no provider-specific model IDs, so the model resolves through whatever backend your Claude Code install is already configured for (Anthropic API, AWS Bedrock, or Google Vertex). No `AWS_REGION` or other Bedrock-specific settings ship with this repository. To target a specific backend or model ID, override `model` and the relevant provider environment variables in `.claude/settings.local.json`.
- **MCP servers (optional)**: This repository ships no `.mcp.json`; the framework runs without MCP servers. To make servers (e.g. `context7` for library/SDK documentation lookups, or the AWS MCP servers) available to the workflow, declare them in a `.mcp.json` at the project root (beside `.claude/`). Credentials flow through environment passthrough — never commit keys — and declared servers are provisioned to the session and **inherited by every agent**; agents that should be prevented from using a server are narrowed via their `tools:` allowlist with fully-qualified `mcp__<server>__<tool>` ids.
...
The AI-DLC framework's full documentation (user guide, harness engineering, developer reference) is not bundled in this repository; `docs/` here holds this crate's own documents (e.g. `DATABASE_SCHEMA.md`). For framework behaviour, the shipped sources under `.claude/` and `/aidlc --help` / `/aidlc --doctor` are the authoritative view.
