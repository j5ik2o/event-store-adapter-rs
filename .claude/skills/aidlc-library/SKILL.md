...
description: >
  Run the AI-DLC workflow with the library scope baked in — no scope
  detection. Full lifecycle for library/crate work, no operation phase. Packaging over `/aidlc --scope library`, which works
  without this skill.
argument-hint: "[description | --status | --stage <slug|#> | --phase <name|#>]"
user-invocable: true
---

# AI-DLC — library scope

Drive the AI-DLC engine with the **library** scope fixed. This is the same
deterministic forwarding loop the `/aidlc` orchestrator runs, with `--scope
library` baked into the first `next` so scope detection is skipped. The
...
1. `directive = bun .claude/tools/aidlc-orchestrate.ts next --scope library $ARGUMENTS`
...
Pass `$ARGUMENTS` through verbatim after `--scope library`; the engine parses
...
- **On genuine new-work, OFFER, never auto-birth.** Surface an
  ...
  `library` (the new work is likely the same flavour that made the user reach for
