...
- When snapshot retention is enabled (`with_keep_snapshot_count` / `with_delete_ttl`), history snapshot rows (seq_nr > 0) are inserted in addition to the seq_nr=0 slot, and excess/expired history rows are deleted client-initiated after each persist. Unlike DynamoDB there is no storage-side TTL; the deletion is always performed by the library.
